...
class ChannelProtect(Filter):
    async def __call__(self, message: Message, bot: Bot):
        ...
        u_status = await bot.get_chat_member(chat_id = int(load_config().tg_bot.channel_id), user_id=message.from_user.id)
    ...
    if args:
        ...
        if not await get_user_from_id(user_id=message.from_user.id):
            ...
            if message.from_user.username:
                 ...
                 await add_user({"id": message.from_user.id, "username": f"None-{message.from_user.id}", "referral_link": link,
                            "referer_id": referrer_id})
        ...
        link = await create_start_link(bot=bot, payload=str(message.from_user.id), encode=True)
        if message.from_user.username:
            await add_user({"id": message.from_user.id, "username": message.from_user.username, "referral_link": link, "status":'None'})
        else:
            await add_user({"id": message.from_user.id, "username": f"None-{message.from_user.id}", "referral_link": link, "status":'None'})
        await user_subscription(message)
...
async def confirm_anketa(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if answer == 'confirm':
        ...
        append_anketa(id_anketa=id_anketa,
                      ...
                      username_refer=callback.from_user.username if callback.from_user.username else f'None-{callback.from_user.id}',
    ...
    try:
        ...
        if user.status != UserStatus.payed:
            ...
            await bot.send_message(chat_id=callback.from_user.id,
                                        text=f'❗️Этому пользователю уже заплатили'
                                                f' (@{await _get_username_from_id(user_to_pay)})  0.15 TON,')
